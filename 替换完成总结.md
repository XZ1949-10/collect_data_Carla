# command_based_data_collection.py 替换完成总结

## ✅ 已完成的替换

### 1. **导入部分**

**替换前**：
```python
from navigation_planner import NavigationPlanner

# 尝试导入 BasicAgent
try:
    from agents.navigation.basic_agent import BasicAgent
    BASIC_AGENT_AVAILABLE = True
except ImportError:
    BASIC_AGENT_AVAILABLE = False
```

**替换后**：
```python
# 导入 agents 模块
try:
    from agents.navigation.basic_agent import BasicAgent
    from agents.navigation.local_planner import RoadOption
    AGENTS_AVAILABLE = True
except ImportError as e:
    AGENTS_AVAILABLE = False
    print(f"⚠️  警告: 无法导入 agents 模块: {e}")
```

---

### 2. **初始化部分**

**删除**：
- `self.navigation_planner = None`
- `self.navigation_planner = NavigationPlanner(self.world, sampling_resolution=2.0)`

**新增**：
- `self.road_option_to_command` - RoadOption 到数值命令的映射字典

---

### 3. **BasicAgent 配置**

**替换前**：
```python
if BASIC_AGENT_AVAILABLE:
    self.agent = BasicAgent(self.vehicle, target_speed=30)
    # 分别配置各项
    if self.ignore_traffic_lights:
        self.agent.ignore_traffic_lights(True)
    # ...
    self.agent.set_destination(destination)
```

**替换后**：
```python
if AGENTS_AVAILABLE:
    opt_dict = {
        'target_speed': 30.0,
        'ignore_traffic_lights': self.ignore_traffic_lights,
        'ignore_stop_signs': self.ignore_signs,
        'ignore_vehicles': (self.ignore_vehicles_percentage > 50),
        'sampling_resolution': 2.0
    }
    
    self.agent = BasicAgent(
        self.vehicle, 
        target_speed=30,
        opt_dict=opt_dict,
        map_inst=self.world.get_map()
    )
    
    # 设置目的地（包含起点）
    start_location = spawn_point.location
    self.agent.set_destination(destination, start_location=start_location)
```

---

### 4. **导航命令获取**

**替换前**：
```python
self.current_command = self.navigation_planner.get_navigation_command(self.vehicle)
```

**替换后**：
```python
self.current_command = self._get_navigation_command()
```

**新增辅助方法**：
```python
def _get_navigation_command(self):
    """从 BasicAgent 的 local_planner 获取当前导航命令"""
    if not AGENTS_AVAILABLE or self.agent is None:
        return 2.0  # Follow
    
    try:
        local_planner = self.agent.get_local_planner()
        if local_planner is None:
            return 2.0
        
        road_option = local_planner.target_road_option
        if road_option is None:
            road_option = RoadOption.LANEFOLLOW
        
        command = self.road_option_to_command.get(road_option, 2.0)
        return command
    except Exception as e:
        print(f"⚠️  获取导航命令失败: {e}")
        return 2.0
```

---

### 5. **路线完成检查**

**替换前**：
```python
if self.navigation_planner.is_route_completed(self.vehicle):
    break
```

**替换后**：
```python
if self._is_route_completed():
    break
```

**新增辅助方法**：
```python
def _is_route_completed(self):
    """检查是否到达目的地"""
    if not AGENTS_AVAILABLE or self.agent is None:
        return False
    
    try:
        return self.agent.done()
    except Exception as e:
        print(f"⚠️  检查路线完成状态失败: {e}")
        return False
```

---

### 6. **变量名统一**

**替换**：
- `BASIC_AGENT_AVAILABLE` → `AGENTS_AVAILABLE`（所有出现的地方）

---

## 📊 替换统计

| 项目 | 替换前 | 替换后 |
|------|--------|--------|
| **导航规划器** | `NavigationPlanner` | `BasicAgent` + `LocalPlanner` |
| **命令获取** | `navigation_planner.get_navigation_command()` | `_get_navigation_command()` |
| **路线完成检查** | `navigation_planner.is_route_completed()` | `_is_route_completed()` |
| **目的地设置** | `navigation_planner.set_destination()` | `agent.set_destination()` |

---

## 🎯 核心改进

1. ✅ **完全使用 agents 模块**：不再依赖自定义的 `NavigationPlanner`
2. ✅ **更规范的配置**：使用 `opt_dict` 统一配置 BasicAgent
3. ✅ **更好的错误处理**：添加了异常处理和降级方案
4. ✅ **代码更清晰**：通过辅助方法封装导航命令获取逻辑

---

## 📝 使用说明

替换后的代码完全兼容原有功能，但：

1. **需要 agents 模块**：确保 `agents/navigation/` 目录存在
2. **命令映射**：RoadOption 自动映射到数值命令（2.0/3.0/4.0/5.0）
3. **降级方案**：如果 agents 不可用，会使用默认命令（Follow）

---

## ✅ 验证

- ✅ 所有 `navigation_planner` 引用已删除
- ✅ 所有 `NavigationPlanner` 引用已删除
- ✅ 所有 `BASIC_AGENT_AVAILABLE` 已替换为 `AGENTS_AVAILABLE`
- ✅ 辅助方法已添加
- ✅ 代码通过 linter 检查

**替换完成！** 🎉

